      ...
      <header className="mx-auto flex w-full max-w-6xl flex-col gap-4 px-5 py-5 sm:flex-row sm:items-center sm:justify-between">
        ...
        {authed ? (
          <div className="relative">
            <button
              className="inline-flex items-center gap-3 rounded-full border border-white/80 bg-white/80 px-4 py-2.5 text-sm font-bold text-[#061a3a] shadow-lg shadow-slate-900/5 backdrop-blur transition hover:-translate-y-0.5"
              ...
              <div className="absolute right-0 top-[calc(100%+0.75rem)] z-50 w-[22rem] overflow-hidden rounded-[1.75rem] border border-white/80 bg-white p-4 shadow-2xl shadow-slate-950/15">
                ...
                <div className="mt-4 space-y-2 text-sm text-slate-600">
                  <div className="flex items-center justify-between rounded-2xl bg-slate-50 px-4 py-3">
                    ...
                    <span className="font-bold text-[#061a3a]">{profile?.member_since ? new Date(profile.member_since).toLocaleDateString() : "N/A"}</span>
                  </div>
                  <div className="flex items-center justify-between rounded-2xl bg-slate-50 px-4 py-3">
                  ...
                  <div className="flex items-center justify-between rounded-2xl bg-slate-50 px-4 py-3">
                    ...
                    <span className="font-bold text-[#061a3a]">{profile?.last_complaint_date ? new Date(profile.last_complaint_date).toLocaleDateString() : "N/A"}</span>
                  </div>
                </div>

                <div className="mt-4 grid gap-2">
                  <NavLink className={navLinkClass} to="/complaints/new" onClick={() => setMenuOpen(false)}>File Complaint</NavLink>
                  <NavLink className={navLinkClass} to="/my-complaints" onClick={() => setMenuOpen(false)}>My Complaints</NavLink>
                  <button
                    onClick={handleLogout}
                    className="rounded-full px-4 py-2 text-left text-sm font-semibold text-rose-700 transition hover:bg-rose-50"
                    type="button"
                  >
                    Logout
                  </button>
